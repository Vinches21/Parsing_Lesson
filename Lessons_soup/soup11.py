...
def calculate_total_price(html: str) -> float:
    tags = soup.find_all('div', class_="book-card")
# tag = soup.find("div", class_= "item_card").select("item .description")
# print(tag)
    res = []
    for tag in tags:
        price = float(tag.find(class_="count price").text.replace("Цена: $", ''))
        col = int(tag.find(class_="count stock").text.replace("Количество на складе: ", ''))
